...
- **Heads up — `Features::Accept` no longer surfaces accept-validation errors for `private:` / `protected:` attributes.** Pre-3.1 the reject path wrote `@__attributes_errors[key]` for any attribute regardless of visibility. Post-fix, private/protected attributes get NO accept-validation surfacing — `attributes_errors`, `attributes_errors?`, `rejected_attributes`, and (for `accept: :strict` users) the ArgumentError raise are all skipped for them. Code that used `attribute :secret, accept: SomeKind, private: true` and inspected `attributes_errors` to validate is now silently broken. Either drop `private:` / `protected:` on attributes you need validation for, or validate internally by reading the ivar.
...
- `Composition::Coercion` now gates on `klass.instance_method(:initialize).arity != 0` instead of `klass.include?(Features::Initialize)`. The arity check covers BOTH `Features::Initialize` includers AND user-defined hash constructors (`def initialize(arg); self.attributes = arg; end`) — the long-standing `u-case` v4 idiom keeps working. Targets that genuinely lack a hash constructor (Object#initialize, arity 0) are still skipped so we don't fall through to `ArgumentError: wrong number of arguments`.
