...
- `Composition::Coercion` no longer calls `klass.new(value)` when the target class lacks the `:initialize` feature — the value passes through to the standard accept check (which then either passes or surfaces a kind-of error). Previously this raised `ArgumentError: wrong number of arguments (given 1, expected 0)` from `Object#initialize`.
- `Micro::Attributes.new(initialize: false)` is rejected upfront with a clear `ArgumentError`. Pre-fix it silently built a class with no hash constructor, so the user's first `Klass.new(hash)` raised an opaque `ArgumentError: wrong number of arguments` from `Object#initialize` instead.
...
- The Coercion bubble (writes `'is invalid'` to the parent's `attributes_errors`) is now gated on `attribute_data[3] == :public`, matching Accept's visibility gate. Private/protected nested-entity keys no longer leak through the parent's `attributes_errors`.
